"""
Pipeline completo da Ancore num arquivo só: login -> N workers em paralelo
-> finalizacao. Um unico comando roda tudo.

Uso:
    python wit_ancore.py                      -> 3 workers, headless
    python wit_ancore.py --num-workers 5      -> 5 workers, headless
    python wit_ancore.py --no-headless        -> abre a janela do navegador
...
Cada worker grava sua fatia de tickets em wit_ancore_tickets_worker{id}.json;
depois que todos terminam, o processo principal une as listas e marca como
'Finalizado' os tickets da Ancore que sumiram da fila "Em Atendimento"
(escopado por empresa='ancore' AND chat='mundiale' — nunca mexe em outras
empresas).
"""
import argparse
import glob
...
EMPRESA_ALVO = "Ancore Atendimento"
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
STORAGE_STATE_FILE = os.path.join(BASE_DIR, "wit_ancore_storage_state.json")
...
def _tickets_file(worker_id):
    return os.path.join(BASE_DIR, f"wit_ancore_tickets_worker{worker_id}.json")
...
def finalizar(num_workers):
    ...
    if not tickets_ativos:
        print("[finalizar] ABORTANDO: lista de tickets ativos veio vazia — "
              "não vou marcar tudo da Ancore como Finalizado por segurança.")
        return
    ...
    """, ('ancore', tuple(tickets_ativos)))
    finalizados = cur.rowcount
    conn.commit()
    conn.close()
    print(f"[finalizar] {finalizados} ticket(s) da Ancore marcado(s) como 'Finalizado'.")
...
def main():
    parser = argparse.ArgumentParser(description="Pipeline completo da Ancore: login -> workers paralelos -> finalização")
